...
describe('Time', () => {
  ...
  ])('should parse valid formats, like "%s"', (input, hour, minute) => {
    expect(Time.fromString(input)).toEqual(Time.fromHour(hour, minute));
  });
  ...
  it.each([
    ...
    'PM:07 00',
    '7:00🕖',
  ])('should fail to parse invalid format, like "%s"', input => {
    expect(Time.fromString(input).value).toBe(NaN);
  });

  it.only.each([
    // Typical cases
    ['should', '24h', '00:00', 0, 0],
    ['should', '24h', '00:01', 0, 1],
    ['should', '24h', '03:10', 3, 10],
    ['should', '24h', '12:00', 12, 0],
    ['should', '24h', '12:01', 12, 1],
    ['should', '24h', '19:10', 19, 10],
    ['should', '24h', '23:59', 23, 59],
    ['should', '12h', '12:00 AM', 0, 0],
    ['should', '12h', '12:01 AM', 0, 1],
    ['should', '12h', '12:00 PM', 12, 0],
    ['should', '12h', '12:01 PM', 12, 1],
    ['should', '12h', '01:00 PM', 13, 0],
    ['should', '12h', '11:59 PM', 23, 59],

    // Mismatch format
    ['should not', '24h', '12:00 AM', NaN, NaN],
    ['should not', '24h', '12:01 AM', NaN, NaN],
    ['should not', '24h', '12:00 PM', NaN, NaN],
    ['should not', '24h', '12:01 PM', NaN, NaN],
    ['should not', '24h', '01:00 PM', NaN, NaN],
    ['should not', '24h', '11:59 PM', NaN, NaN],
    ['should not', '12h', '00:00', NaN, NaN],
    ['should not', '12h', '12:01', NaN, NaN],
    ['should not', '12h', '19:10', NaN, NaN],
    ['should not', '12h', '23:59', NaN, NaN],
  ])('%s parse values for %s format like %s', (_, format, input, hour, minute) => {
    const parsed = Time.fromString(input, format);
    expect(parsed.hour).toBe(hour);
    expect(parsed.minute).toBe(minute);
  });
