...
export default function TimePicker({value, startTime, step, min, max, timeFormat, onChange}) {
  const [internalValue, setInternalValue] = useState(formatTimeString(value, timeFormat));
  ...
  const handleChange = ev => {
    ...
    setInternalValue(newValue, Time.fromString(newValue, timeFormat).toString());
    onChange(newValue && Time.fromString(newValue, timeFormat).toString());
  };

  const handleBlur = ev => {
    const maybeFixedValue = formatTimeString(ev.target.value, timeFormat, true);
    ...
    setInternalValue(maybeFixedValue);
  };
